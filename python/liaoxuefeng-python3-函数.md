## [函数](https://www.liaoxuefeng.com/wiki/1016959663602400/1017105451316128)

可以在交互式命令行通过`help(abs)`查看`abs`函数的帮助信息。

传入的参数数量不对，会报`TypeError`的错误
...
二是如何设置默认参数。当函数有多个参数时，把变化大的参数放前面，变化小的参数放后面。变化小的参数就可以作为默认参数。降低了函数调用的难度，而一旦需要更复杂的调用时，又可以传递更多的参数来实现
